...
HIVE="beeline -u 'jdbc:hive2://ia0304.halxg.cloudera.com:10000/default;principal=hive/ia0304.halxg.cloudera.com@HALXG.CLOUDERA.COM'"
...
do
	COMMAND="$HIVE  -i settings/load-partitioned.sql -f ddl-tpcds/bin_partitioned/${t}.sql \
	    --hivevar DB=${DATABASE} --hivevar SOURCE=tpcds_${SCALE}_text \
            --hivevar SCALE=${SCALE} \
...
do
	COMMAND="$HIVE  -i settings/load-partitioned.sql -f ddl-tpcds/bin_partitioned/${t}.sql \
	    --hivevar DB=${DATABASE} \
            --hivevar SCALE=${SCALE} \
	    --hivevar SOURCE=tpcds_${SCALE}_text --hivevar BUCKETS=${BUCKETS} \
